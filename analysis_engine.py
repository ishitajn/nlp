"""
This module orchestrates the entire analysis pipeline, calling each specialized
engine in the correct order and assembling their results.
"""
...
def run_full_analysis(
    ...
    """
    Orchestrates the full analysis pipeline using the provided services.
    """
    # 1. Topic Analysis
    topic_map, profile_topics = identify_and_canonicalize_topics(
        services, processed_turns, their_profile, use_enhanced_nlp
    )
    ...
    categorized_topics = score_and_categorize_topics(
        services, topic_map, profile_topics, focus_topic, topic_salience, use_enhanced_nlp
    )

    # 2. Behavioral, Contextual, and Memory Analysis
    behavioral_analysis = analyze_conversation_behavior(services, processed_turns, use_enhanced_nlp)
    contextual_and_memory_features = extract_contextual_features(
        services, processed_turns, categorized_topics, my_profile, their_profile, use_enhanced_nlp
    )

    # 3. Detailed Last Message Analysis
    last_turn = processed_turns[-1] if processed_turns else None
    last_message_analysis = analyze_last_message_details(services, last_turn)

    # 4. Assemble the final analysis object
    return {
        "categorized_topics": categorized_topics,
        "recent_topics": recent_topics,
        "contextual_features": contextual_and_memory_features,
