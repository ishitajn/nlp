...
class ContextualSuggestionEngine:
    ...
    def _find_semantically_similar_topics(self, seed_topics: List[str], category: str) -> List[str]:
        ...
        seed_embeddings = [
            np.mean(embedder_service.encode_cached([turn['content'] for turn in self.topic_map.get(seed.lower(), [])]), axis=0)
    ...
    def generate(self, behavioral_analysis: Dict[str, Any]) -> Dict[str, List[str]]:
        if self.use_enhanced_nlp:
            suggestions = self._generate_enhanced_suggestions(behavioral_analysis)
        else:
    ...
    def _generate_enhanced_suggestions(self, behavioral_analysis: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        if profile_text.strip():
            ...
            if "neutral" not in suggestions: suggestions["neutral"] = []
